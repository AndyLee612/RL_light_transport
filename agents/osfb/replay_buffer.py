"""Module defining the replay buffer for the One-Step FB agent."""
...
class OSFBReplayBuffer(OfflineReplayBuffer):
    """One-step forward backward replay buffer.

    Identical to FBReplayBuffer but also returns next_actions in
    sample_task_inference_transitions for z inference with the
    action-conditioned backward representation.
    """
    ...
    def sample_task_inference_transitions(
        ...
        """
        Sample transitions from the replay buffer for OSFB task inference.
        Returns observations, actions, and rewards (actions are needed because
        the one-step FB backward representation is action-conditioned).
        ...
        Returns:
            ...
            z_inf_actions: actions for task inference
        ...
        batch_actions = self.storage["actions"][batch_indices]
        ...
        # return actions as tensor (needed for action-conditioned backward repr)
        z_inf_actions = batch_actions

        return z_inf_observations, z_inf_actions, z_inf_rewards


class OnlineOSFBReplayBuffer(OSFBReplayBuffer):
    """
    Replay buffer for one-step FB agent that initially loads an offline
    dataset, but allows data obtained from online interaction to augment it.
    Stores next_actions for both offline and online data.
    """
    ...
    def __init__(
        ...
        self.online_actions = np.empty(
            (capacity, action_length),
            dtype=np.float32,
        )

        self.online_next_actions = np.empty(
            (capacity, action_length),
            dtype=np.float32,
        )
    ...
    def add(
        ...
        Args:
            ...
            next_action: array of shape [action_length]
        ...
        np.copyto(self.online_next_actions[self.current_memory_index], next_action)
    ...
    def sample(self, batch_size: int):
        ...
        next_actions = torch.cat(
            (
                self.storage["next_actions"][offline_sample_indices],
                torch.as_tensor(
                    self.online_next_actions[online_sample_indices], device=self.device
                ),
            ),
        )
        rewards = torch.cat(
